...
│   │
│   ├── core/                     # 核心引擎 — "大脑"
│   │   ├── chat.py               # 聊天主循环 / agent loop
│   │   ├── context.py            # 上下文窗口管理（裁剪、折叠、过滤）
│   │   └── session.py            # 会话生命周期（ChatSession、SessionManager）
│   │
│   ├── tools/                    # 工具系统 — "双手"
...
│   │   ├── get_current_time.py   # 工具：获取时间
│   │   └── calculate.py          # 工具：计算器
│   │
│   ├── services/                 # 基础设施 — "神经"
│   │   ├── llm.py                # LLM适配器（OpenAI兼容格式）
│   │   ├── history.py            # SQLite持久化（会话、消息、摘要）
│   │   ├── memory.py             # 记忆系统（摘要生成、记忆注入）
...
│   │   └── character.py          # 角色卡片加载（校验、列表、读取）
│   │
│   └── types/                    # 类型定义 — "词汇"
│       └── messages.py           # 消息类型、元数据、折叠常量
│
├── api/                          # FastAPI HTTP接口
│   ├── main.py                   # 应用入口、CORS、路由注册
│   └── routes/
│       ├── chat.py               # 聊天端点（send/stream/history）
...
| 文件 | 职责 | 关键函数 |
|------|------|----------|
| `chat.py` | 聊天主循环，工具调用流程 | `chat_stream()`, `chat_non_stream()`, `validate_tool_call()` |
| `context.py` | 上下文窗口管理 | `trim_messages()`, `fold_tool_calls()`, `filter_for_ai()` |
| `session.py` | 会话生命周期 | `ChatSession`, `SessionManager`, `get_session_manager()` |
...
| `get_current_time.py` | 获取时间 | `execute()` |
...
| `memory.py` | 记忆系统 | `generate_summary()`, `get_memory_context()`, `summarize_session()` |

### prompt/ — 提示词构建

| 文件 | 职责 | 关键函数 |
|------|------|----------|
| `builder.py` | 提示词拼接 | `build_system_prompt()`, `build_messages()` |
