...
**最后更新**：2026-05-18（多编辑器架构：三表独立 + Scene独立TipTap + ManuscriptView平铺列表 + Plan视图）
...
│   │   │   ├── writing.py        # T11 写作模式存储（writing.db：作品/Act/Chapter/Scene/设定 CRUD，三表独立，Scene存PM JSON，事务保护）
│   │   │   └── writing_snapshot.py # T33 快照存储（v2: acts/chapters/scenes树形快照，v1兼容恢复，共享 writing.py 连接/锁）
...
│       │   ├── writing/           # T11 写作模式组件（多编辑器架构）
│       │   │   ├── ChapterSidebar.tsx  # 左栏：作品列表+章节管理+世界观入口
│       │   │   ├── WritingEditor.tsx    # 中间：工具栏+ManuscriptView容器（纯React，编辑器在SceneEditor中）
│       │   │   ├── AiWritingPanel.tsx  # 右栏：AI 助手（对话/续写/润色/扩写/精简，WebSocket 流式桩）
│       │   │   ├── ManuscriptView.tsx  # NEW：平铺列表渲染器（Acts→Chapters→Scenes flat list）
│       │   │   ├── SceneEditor.tsx     # NEW：独立TipTap编辑器（每个Scene一个实例，500ms debounce PM JSON保存）
│       │   │   ├── ActHeader.tsx       # NEW：纯React Act标题行（border-top: 3.5px + 64px padding）
│       │   │   ├── ChapterHeader.tsx   # NEW：纯React Chapter标题行（border-top: 0.7px + 2.5em padding）
│       │   │   ├── SceneSeparator.tsx  # NEW：场景间分隔线（纯React组件，不在编辑器内）
│       │   │   ├── InsertButton.tsx    # NEW：虚线插入按钮（+Scene/+Chapter/+Act）
│       │   │   ├── PlanView.tsx        # NEW：Plan视图（@dnd-kit树形拖拽，展开/折叠）
│       │   │   ├── PlanTreeItem.tsx    # NEW：可拖拽树节点（GripVertical + expand + delete）
│       │   ├── rpg/              # RPG 模式组件
...
│       ├── components/           # UI 组件（ChatPanel, ActivityBar, SidePanel, Popover, MarkdownContent, CommandPalette, MemoryWindow, GraphWindow, GraphEditor, RpgPanel, FloatingLayerHost, TdbFileTree, ResizablePanel, PermissionTree, panels/）+ editors/（TipTap扩展：GhostText, FocusMode, SlashCommand, SceneBeat, SceneBeatView, SectionBlock, SectionBlockView, CodexHighlight, GenerationBar, GenerateTextDialog, BeatContextMenu, SelectionToolbar。旧PM NodeView节点已删除）
│       ├── pages/                # 页面组件（WorldBookList, WorldBookEditor, SkillList, SkillEditor, AvatarManager, ConfigList, ConfigEditor, DebugWindowPage, ToolTipsPage, ThinkingClustersPage, PermissionPage）
...
| `GET` | `/writing/projects/{id}/acts` | Act 列表 |
| `POST` | `/writing/acts` | 创建 Act |
| `PATCH` | `/writing/acts/{id}` | 更新 Act |
| `DELETE` | `/writing/acts/{id}` | 删除 Act |
| `POST` | `/writing/acts/reorder` | 重排 Act |
| `GET` | `/writing/projects/{id}/chapters` | 章节列表 |
| `POST` | `/writing/chapters` | 创建章节（需 act_id） |
| `PATCH` | `/writing/chapters/{id}` | 更新章节 |
| `DELETE` | `/writing/chapters/{id}` | 删除章节（级联 Scene） |
| `GET` | `/writing/chapters/{id}/scenes` | Scene 列表 |
| `POST` | `/writing/scenes` | 创建 Scene |
| `PATCH` | `/writing/scenes/{id}` | 更新 Scene（content/summary/subtitle） |
| `DELETE` | `/writing/scenes/{id}` | 删除 Scene |
| `POST` | `/writing/scenes/reorder` | 重排 Scene |
| `GET` | `/writing/projects/{id}/manuscript` | 完整手稿（树形结构） |
