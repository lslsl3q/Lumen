...
function InfoPanel() {
  ...
  return (
    <div className="w-48 flex flex-col bg-[var(--color-bg-deep)] border-l border-[var(--color-border)]">
      {/* 标题 */}
      <div className="h-10 flex items-center gap-2 px-3 border-b border-[var(--color-border)]">
      ...
      <div className="flex-1 overflow-y-auto py-2 px-2">
        {online.length === 0 && offline.length === 0 ? (
          <div className="flex items-center justify-center h-24">
            <p className="text-[10px] text-[#444]">暂无成员</p>
          </div>
        ) : (
          <>
            {online.map((member) => (
              <div
                ...
                className="flex items-center gap-2 px-2 py-1.5 rounded-md
                  hover:bg-[#ffffff06] cursor-pointer transition-colors"
              >
                <div className="relative">
                  ...
                  <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full
                    bg-emerald-400 border-2 border-[var(--color-bg-deep)]" />
                </div>
            ...
            {offline.length > 0 && (
              <>
                <div className="px-2 mt-3 mb-1">
                  <span className="text-[9px] text-[#444] uppercase tracking-wider">离线 — {offline.length}</span>
                </div>
                {offline.map((member) => (
                  <div
                    key={member.id}
                  ...
                  >
                    <div className="relative">
                      ...
                      <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full
                        bg-[#555] border-2 border-[var(--color-bg-deep)]" />
                    </div>
