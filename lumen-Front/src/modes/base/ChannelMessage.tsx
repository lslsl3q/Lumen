  ...
  if (message.type === 'system') {
    return (
      <div className="px-3 py-1">
        <span className="text-[11px] italic text-[#555]">
          — {message.content} —
        </span>
      </div>
    );
    ...
    return (
      <div className="px-3 py-1.5 hover:bg-[#ffffff04] rounded-md">
        <div className="flex items-baseline gap-2">
          <span className="text-xs font-semibold text-[#CC7C5E]">你</span>
          <span className="text-xs text-[#555]">
            {new Date(message.timestamp).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}
          </span>
        </div>
        <div className="text-[13px] text-[#bbb] mt-0.5 leading-relaxed">{message.content}</div>
      </div>
    );
  }
