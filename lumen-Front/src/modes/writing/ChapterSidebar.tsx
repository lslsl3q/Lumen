...
export function ChapterSidebar() {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#141413] border-r border-[#2a2926] select-none">
      {/* 顶部：作品列表 */}
      <div className="p-3 border-b border-[#2a2926]">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className="p-1 rounded hover:bg-[#2a2926] text-slate-500 hover:text-slate-300 cursor-pointer transition-colors"
            title="新建作品"
          >
              ...
              <div
                ...
                className={`flex items-center gap-1.5 px-2 py-2 rounded-md text-[13px] cursor-pointer group transition-colors
                  ${isActive
                    ...
                    : "text-slate-400 hover:bg-[#1f1f1c] hover:text-slate-200"
                  }`}
              >
                {isExpanded ? (
                ...
                <button
                  ...
                  className="p-0.5 rounded opacity-0 group-hover:opacity-100 text-slate-600 hover:text-red-400 cursor-pointer transition-opacity"
                >
      ...
      {activeProject && (
        <div className="flex-1 overflow-y-auto scrollbar-lumen">
          ...
          {chapters.length === 0 && (
            <p className="text-[11px] text-slate-600 italic px-5 py-3">点击 + 创建第一章</p>
          )}
              ...
              >
                {editingChapterId === ch.id ? (
                  ...
                  <span className="truncate flex-1">
                    <span className="text-slate-600 mr-1.5 text-[11px]">{idx + 1}.</span>
                    {ch.title}
                  </span>
                )}
                <span className="text-[10px] text-slate-600 mr-1 flex-shrink-0">{ch.word_count ?? 0}字</span>
                <button
                  onClick={async (e) => {
                    e.stopPropagation();
                  ...
                  className="p-0.5 rounded opacity-0 group-hover:opacity-100 text-slate-600 hover:text-red-400 cursor-pointer transition-opacity flex-shrink-0"
                >
                  <Trash2 className="w-3 h-3" />
                </button>
              </div>
      ...
      {/* 底部：导出 + 世界观入口 */}
      {activeProject && (
        <div className="p-2 border-t border-[#2a2926] flex-shrink-0 space-y-0.5">
          {/* 导出 */}
          <div className="flex gap-1">
            <a
              href={getExportUrl(activeProjectId!, "txt")}
              download
              className="flex items-center gap-2 flex-1 px-3 py-2 rounded-md text-[13px] text-slate-500 hover:text-slate-300 hover:bg-[#1f1f1c] cursor-pointer transition-colors no-underline"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 w-full px-3 py-2 rounded-md text-[13px] text-slate-500 hover:text-slate-300 hover:bg-[#1f1f1c] cursor-pointer transition-colors"
          >
...
function SettingsList() {
  ...
  return (
    <div className="mt-1 px-1 space-y-0.5 max-h-[300px] overflow-y-auto scrollbar-lumen">
      {settings.length === 0 && (
        <p className="text-[10px] text-slate-600 italic px-3 py-1">暂无设定</p>
      )}
      {settings.map((s) => {
        ...
        return (
          <div key={s.id} className="rounded-md overflow-hidden">
            <div
              ...
              className="flex items-center justify-between px-3 py-1.5 text-[12px] text-slate-400 hover:bg-[#1f1f1c] group transition-colors cursor-pointer"
            >
              <span className="flex items-center gap-1.5 truncate flex-1">
                <span className="text-[10px] text-slate-600">{CATEGORY_LABELS[s.category] ?? s.category}</span>
            ...
            {isExpanded && (
              <div className="px-3 pb-2 space-y-1.5">
                <input
                  ...
                  className="w-full bg-[#1f1f1c] border border-[#2a2926] rounded px-2 py-1 text-[12px] text-slate-200 outline-none focus:border-amber-400/30"
                  placeholder="设定名称"
                />
                <select
                  value={s.category}
                  ...
                  className="w-full bg-[#1f1f1c] border border-[#2a2926] rounded px-2 py-1 text-[11px] text-slate-400 outline-none focus:border-amber-400/30"
                >
      ...
      <button
        ...
        className="flex items-center gap-1 w-full px-3 py-1.5 rounded-md text-[11px] text-slate-600 hover:text-slate-400 hover:bg-[#1f1f1c] cursor-pointer transition-colors"
      >
