...
function WorkspacesEditor({ data, onSave, isSaving }: WorkspacesEditorProps) {
  ...
  return (
    <div className="space-y-6">
      {/* 工作区路径列表 */}
      <section>
        <h3 className="text-sm text-text-secondary mb-3">工作区路径</h3>
        ...
        <div className="space-y-2">
          {workspaces.map((path, i) => (
            <div
              ...
              "
            >
              <span className="text-sm text-text-primary font-mono">{path}</span>
              <button
      ...
      <section className="space-y-4">
        <h3 className="text-sm text-text-secondary">其他设置</h3>

        {/* 只读模式 */}
        <label className="flex items-center justify-between px-4 py-3 rounded-lg bg-surface-elevated border border-border-subtle">
          <div>
            <div className="text-sm text-text-primary">只读模式</div>
            <div className="text-xs text-text-muted">开启后 AI 只能读取文件，不能写入</div>
          </div>
          <button
        ...
        {/* 文件大小限制 */}
        <div className="px-4 py-3 rounded-lg bg-surface-elevated border border-border-subtle">
          <div className="text-sm text-text-primary mb-2">单文件大小上限（MB）</div>
          <input
            type="number"
            min={1}
            max={100}
