...
export default function DebugDrawer({ open, onClose, layers, totalTokens, contextSize, recallLog, reactTrace }: DebugDrawerProps) {
  ...
  return (
    <div className="absolute right-10 top-0 bottom-0 w-96 flex flex-col bg-slate-950/98 backdrop-blur-sm border-l border-slate-800/30 shadow-[-6px_0_20px_rgba(0,0,0,0.35)] z-40 animate-slide-in">
      {/* 头部 — 与 ContextPanel 统一 */}
      <div className="flex items-center justify-between px-4 py-3 bg-slate-900/30 border-b border-slate-800/30">
        <span className="text-xs font-medium tracking-wide text-slate-400 font-display">调试面板</span>
        <button
          onClick={onClose}
