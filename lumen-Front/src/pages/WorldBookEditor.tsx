...
function WorldBookEditor({ worldBookId, onBack }: WorldBookEditorProps) {
  ...
  if (isLoading) {
    return <div className="h-full bg-surface-deep text-text-primary flex items-center justify-center">加载中...</div>;
  }

  // 复用样式常量
  const inputCls = "w-full px-4 py-2.5 rounded-lg text-sm bg-border-subtle border border-border-subtle text-text-primary placeholder-slate-600 focus:outline-hidden focus:border-primary/40 transition-all";
  const numInputCls = "w-24 px-3 py-1.5 rounded-lg text-sm bg-border-subtle border border-border-subtle text-text-primary focus:outline-hidden focus:border-primary/40 transition-all";
  const sectionTitleCls = "text-sm text-text-secondary mb-3";
      ...
      <form onSubmit={handleSubmit} className="px-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <section className="space-y-4">
            ...
            {/* 主关键词 */}
            <div>
              <label className="block text-xs text-text-muted mb-1.5">主关键词（任一命中即触发）</label>
              <div className="flex flex-wrap gap-1.5 mb-2 min-h-[28px]">
                {form.keywords.map((kw, i) => (
                  <span key={kw} className={tagCls('amber')}>
                    {kw}
                    <button onClick={() => handleRemoveKeyword(i)} className="text-primary hover:text-primary">×</button>
                  </span>
                ))}
                {form.keywords.length === 0 && <span className="text-text-muted text-xs">暂无</span>}
              </div>
                ...
                <div className="space-y-3 pl-1 border-l-2 border-slate-800 ml-1">
                  ...
                  <div>
                    <div className="flex flex-wrap gap-1.5 mb-2 min-h-[24px]">
                      ...
                      {form.secondary_keywords.length === 0 && <span className="text-text-muted text-xs">暂无次关键词</span>}
                    </div>
                    <div className="flex gap-2">
            ...
            <div>
              <label className="block text-xs text-text-muted mb-1">扫描深度（最近N条消息）</label>
          ...
          <section className="space-y-4">
            ...
            <div>
              <label className="block text-xs text-text-muted mb-1">注入深度（1-10，越小越优先）</label>
            ...
            <div>
              <label className="block text-xs text-text-muted mb-1">优先级（越小越优先）</label>
            ...
            {/* 启用开关 */}
            <label className="flex items-center justify-between px-4 py-3 rounded-lg bg-border-subtle border border-border-subtle">
              <div>
                <div className="text-sm text-text-primary">启用此条目</div>
        ...
        <section className="space-y-2">
          ...
          <textarea value={form.content} onChange={e => setForm({ ...form, content: e.target.value })}
            ...
            rows={10}
            className="w-full px-4 py-3 rounded-lg text-sm bg-border-subtle border border-border-subtle text-text-primary placeholder-slate-600 focus:outline-hidden focus:border-primary/40 transition-all resize-y" />
        </section>

        {/* 底部操作栏 */}
        <div className="flex justify-end gap-3 pt-4 pb-8 border-t border-border-default">
          <button type="button" onClick={() => goBack()} className="px-5 py-2.5 rounded-lg text-sm text-text-secondary hover:text-text-primary transition-colors">
            取消
          </button>
          <button type="submit" disabled={isSaving}
            className="px-5 py-2.5 rounded-lg text-sm font-medium bg-primary/10 border border-primary/30 text-primary hover:bg-primary/20 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
            {isSaving ? '保存中...' : '保存'}
          </button>
        </div>
      </form>
