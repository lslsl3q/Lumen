...
function WorldBookEditor({ worldBookId, onBack }: WorldBookEditorProps) {
  ...
  if (isLoading) {
    return <div className="h-full bg-[var(--color-bg-deep)] text-[var(--color-text-primary)] flex items-center justify-center">加载中...</div>;
  }
    ...
    <div className="h-full bg-[var(--color-bg-deep)] text-[var(--color-text-primary)]">
      ...
      <div className="flex items-center justify-between mb-6 px-6 py-4">
        <div className="flex items-center gap-4">
          <button onClick={() => goBack()} className="text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors">
            ← 返回
          </button>
      ...
      <form onSubmit={handleSubmit} className="px-6 space-y-6">
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-[var(--color-text-muted)] mb-1">名称</label>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <section className="space-y-4">
            ...
            {/* 主关键词 */}
            <div>
              <label className="block text-xs text-[var(--color-text-muted)] mb-1.5">主关键词（任一命中即触发）</label>
            ...
            {/* 次关键词 + selective */}
            <div>
              <label className="flex items-center gap-2 text-xs text-[var(--color-text-muted)] mb-2 cursor-pointer">
                ...
                <div className="space-y-3 pl-1 border-l-2 border-slate-800 ml-1">
                  ...
                  <div>
                    <div className="flex flex-wrap gap-1.5 mb-2 min-h-[24px]">
                      ...
                      {form.secondary_keywords.length === 0 && <span className="text-[var(--color-text-muted)] text-xs">暂无次关键词</span>}
                    </div>
                    <div className="flex gap-2">
            ...
            <div>
              <label className="block text-xs text-[var(--color-text-muted)] mb-1">扫描深度（最近N条消息）</label>
          ...
          <section className="space-y-4">
            ...
            <div>
              <label className="block text-xs text-[var(--color-text-muted)] mb-1">注入深度（1-10，越小越优先）</label>
            ...
            <div>
              <label className="block text-xs text-[var(--color-text-muted)] mb-1">优先级（越小越优先）</label>
            ...
            {/* 启用开关 */}
            <label className="flex items-center justify-between px-4 py-3 rounded-lg bg-[var(--color-border-subtle)] border border-[var(--color-border-subtle)]">
              <div>
                <div className="text-sm text-[var(--color-text-primary)]">启用此条目</div>
        ...
        {/* 底部操作栏 */}
        <div className="flex justify-end gap-3 pt-4 pb-8 border-t border-[var(--color-border)]">
          <button type="button" onClick={() => goBack()} className="px-5 py-2.5 rounded-lg text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors">
            取消
          </button>
